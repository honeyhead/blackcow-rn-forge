...
assert_file_contains "$COMMAND_DIR/rn-yolo-build.md" '이번 MVP에서는 수익화 없음`이 보이면 유틸리티 앱으로 간주' 'rn-yolo-build should detect utility apps from plan artifacts'
assert_file_contains "$COMMAND_DIR/rn-yolo-build.md" '유틸리티 앱이면 billing / ads 연결을 억지로 추가하지 말고' 'rn-yolo-build should avoid forcing billing or ads for utility apps'
